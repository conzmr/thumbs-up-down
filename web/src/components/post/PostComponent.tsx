...
// import SvgThumbUpBorder from '@material-ui/icons/rBorder'
...
export class PostComponent extends Component<IPostComponentProps, IPostComponentState> {
  ...
  render () {
    ...
    console.log('jejejejjejehjdssss')
    console.log(post)
    // Define variables
    return (
      <Card key={`post-component-${id}`}>
        <CardHeader
          title={reactStringReplace(courseName, /#(\w+)/g, (match: string, i: string) => (
            <NavLink
              style={{ color: 'green' }}
              key={match + i}
              to={`/tag/${match}`}
        ...
        <CardActions>
          <div className={classes.vote}>
            <IconButton
              ...
              <Checkbox
                className={classes.iconButton}
                checkedIcon={<SvgThumbUp style={{ fill: 'rgb(0, 188, 212)' }} />}
                icon={<SvgThumbUp style={{ fill: 'gray' }} />}
